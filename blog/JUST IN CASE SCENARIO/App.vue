        ...
        <div class="payment-methods">
            <label>
                <input type="radio" v-model="paymentMethod" value="Cash" 
                       @change="showOnlinePaymentOptions = false" /> Cash
            </label>
            <label>
                <input type="radio" v-model="paymentMethod" value="Online Payment" 
                       @change="showOnlinePaymentOptions = true" /> Online Payment
            </label>
            <label>
                <input type="radio" v-model="paymentMethod" value="Ticket" 
                       @change="showOnlinePaymentOptions = false" /> Ticket
            </label>
        </div>

        <!-- Online Payment Options -->
        <div v-if="showOnlinePaymentOptions" class="online-payment-options">
            <h3>Select Online Payment Method:</h3>
            <label>
                <input type="radio" v-model="selectedOnlinePaymentMethod" value="GCash" /> GCash
            </label>
            <label>
                <input type="radio" v-model="selectedOnlinePaymentMethod" value="PayPal" /> PayPal
            </label>
			<label>
                <input type="radio" v-model="selectedOnlinePaymentMethod" value="PayMaya" /> PayMaya
            </label>
        </div>
        <div class="button-container">
...
<div v-if="showSuccessPopup" class="popup-overlay active">
    <div class="popup-box">
        ...
        <p><strong>Movie:</strong> {{ purchasedTickets[purchasedTickets.length - 1].name }}</p>
        <p><strong>Seats:</strong> {{ purchasedTickets[purchasedTickets.length - 1].seats.join(', ') }}</p>
        <p><strong>Ticket Numbers:</strong> {{ purchasedTickets[purchasedTickets.length - 1].ticketNumbers.join(', ') }}</p>
        <p><strong>Total Price:</strong> ₱{{ purchasedTickets[purchasedTickets.length - 1].totalPrice }}</p>
		<div class="button-container">
...
export default {
    data() {
        return {
			...
			selectedOnlinePaymentMethod: '', // Store selected online payment option
			showOnlinePaymentOptions: false, // Flag to toggle payment options visibility
            showPaymentPopup: false,
            showSuccessPopup: false,
			ticketCounter: 1, // Start ticket counter at 1
...
confirmPayment() {
    ...
    if (this.paymentMethod === 'Online Payment' && !this.selectedOnlinePaymentMethod) {
    ...
    const paymentStatus = this.paymentMethod === 'Cash'
        ...
        ? `Paid via ${this.selectedOnlinePaymentMethod}`
        : 'Voucher Ticket';

    // Generate Ticket Numbers
    const ticketNumbers = this.selectedSeats.map(() => {
        const formattedTicketNumber = String(this.ticketCounter).padStart(5, '0');
        this.ticketCounter++;
...
//you can reset ticketCounter manually at an appropriate point in your application flow.
/*resetTicketCounter() {
    this.ticketCounter = 1; // Reset counter
}*/
...
.popup-box .button-container {
    ...
    width: 100%; /* Ensure the buttons span the full width of the popup */
    margin-top: 30px; /* Add spacing above the buttons */
}

.popup-box .button-container button {
    flex: 1; /* Make buttons of equal width */
    padding: 10px 15px;
    font-size: 16px;
    cursor: pointer;
    border: none;
...
.popup-box .button-container button:hover {
